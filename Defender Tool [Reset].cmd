...
title Restore Defender Tool ^(thanks freddie-o, geepnozeex, St1ckys, Wesmer, Bau^)
...
:: Check if the Killer script exists and execute it
if exist "%~dp0Defender Tool [Killer].ps1" (
    echo [+] Executing Kernel Killer via RTCore64...
    powershell -nop -ep bypass -file "%~dp0Defender Tool [Killer].ps1"
) else (
    echo [!] Warning: Defender Tool [Killer].ps1 not found in script directory.
    echo [!] Skipping Kernel-level termination...
)
...
echo == Restore Tamper Protection
...
>nul 2>&1 powershell -ep bypass -nop -c "gwmi Win32_BaseService|where Name -Match 'SecurityHealthService|WinDefend|Sense|WdBoot|WdFilter|WdNisSvc|WdNisDrv|wscsvc|SgrmBroker|MdCoreSvc'|foreach {$_.StartService()}"
rem glitch ... using PS ... it work even with tampre on ... idea from geepnozeex

echo == Start Windows Defender Services [via SC]
for %%A IN (SecurityHealthService,WinDefend,WdBoot,WdFilter,Sense,WdNisDrv,WdNisSvc,wscsvc,SgrmBroker,MdCoreSvc) do (
